...
// Splits a headline into words for a staggered reveal.
    ...
    <h1 className={className}>
      ...
      <span className="inline-block overflow-hidden align-bottom">
        <motion.span
          ...
          initial={{ y: "110%" }}
          animate={{ y: "0%" }}
          transition={{ duration: 0.6, delay: 0.05 * words.length, ease: [0.22, 1, 0.36, 1] }}
        >
          {highlight}
        </motion.span>
      </span>
...
// Magnetic hover: the button nudges slightly toward the cursor.
function MagneticLink({ to, href, className, children }) {
  ...
  return (
    <motion.div
      ...
      className="inline-block"
    >
      <Comp {...linkProps} className={className}>
        {children}
      </Comp>
    </motion.div>
  );
...
function CountUpStat({ value, label, delay }) {
  ...
  const target = isNumeric ? parseInt(value.replace(/[,+]/g, ""), 10) : null;
  ...
  return (
    ...
    >
      <dt className="font-mono text-2xl font-medium text-[#141414]">{display}</dt>
...
export default function Hero() {
  ...
  return (
    ...
    >
      {/* animated gradient blob, drifts toward the cursor */}
      <motion.div
        aria-hidden="true"
      ...
      >
        <path d="M-100 650 C 250 500, 400 700, 700 480 S 1100 250, 1550 350" stroke="#007A1F" strokeWidth="2" strokeDasharray="6 10" fill="none" />
        <path d="M-100 200 C 300 300, 500 100, 850 220 S 1200 450, 1550 300" stroke="#7CE629" strokeWidth="2" strokeDasharray="6 10" fill="none" />
      </svg>
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-16 px-6 lg:grid-cols-2 lg:px-12">
        <div>
          ...
          >
            <MagneticLink
              ...
              className="inline-block rounded-xl bg-[#007A1F] px-7 py-3.5 font-body text-sm font-semibold text-white shadow-sm transition-colors duration-300 hover:bg-[#006619] hover:shadow-lg hover:shadow-[#007A1F]/20"
            >
              List your business
            </MagneticLink>
            <MagneticLink
              href="#what-we-do"
              className="inline-block rounded-xl border border-[#CDD5CE] bg-white px-7 py-3.5 font-body text-sm font-semibold text-[#141414] transition-colors duration-300 hover:border-[#007A1F] hover:text-[#007A1F]"
            >
              See how it works
            </MagneticLink>
          </motion.div>
          ...
          <dl className="mt-14 grid max-w-md grid-cols-3 gap-6 border-t border-[#E1E5E1] pt-8">
            {STATS.map(([value, label], i) => (
              <CountUpStat key={label} value={value} label={label} delay={0.7 + i * 0.1} />
            ))}
          </dl>
        </div>
        ...
        >
          ...
          >
            <p className="font-mono text-xs uppercase tracking-wide text-[#858585]">Verified this week</p>
            <p className="mt-1 font-display text-lg text-[#141414]">212 new businesses</p>
          </motion.div>
        </motion.div>
      </div>
    </section>
